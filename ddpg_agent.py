...
class DDPGAgent(object):
    ...
    def nn_actor_model(self):
        ...
        results = Dense(self.env.action_space.shape[0], activation='tanh')(hidden_layer2)
    ...
    def make_actor_train_fn(self):
        ...
        train_fn = kb.function(
            ...
            outputs=[self.actor_model.output],
            updates=update_op
        )
        return train_fn
    ...
    def actor_critic_replay(self):
        ...
        _ = self.actor_train_fn([states])

        return crt_loss

    def policy_action(self, state):
        action_val = self.ACTION_SCALE*self.take_action(state)
...
def evaluation(env, agent, i_episode):
...
def train():
    ...
    for i_episode in range(nb_episode):
        ...
        evaluation(agent=agent, env=env, i_episode=i_episode)
